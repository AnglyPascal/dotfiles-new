...
c.url.searchengines = {
    ...
    "lg":
    "http://libgen.is/search.php?req={}&lg_topic=libgen&open=0&view=simple&res=25&phrase=1&column=def",
...
c.aliases = {
    ...
    "q": "close",
    "mpv":
    "spawn --detach --userscript /home/ahsan/.config/qutebrowser/userscripts/view_in_mpv",
    "pass": "spawn -d pass -c",
...
config.bind(
    ";dt",
    "set downloads.location.prompt true ;; set downloads.remove_finished -1")
config.bind(
    ";df",
    "set downloads.location.prompt false ;; set downloads.remove_finished 0")
...
config.bind(
    ";M",
    "spawn --detach mpv --no-mute --no-terminal -force-window=immediate " +
    '--keep-open=yes "{url}"',
)
config.bind(
    ";m",
    "hint links spawn --detach mpv --no-mute --no-terminal -force-window=immediate "
...
config.bind("<ctrl+shift+r>",
            "config-source /home/ahsan/.config/qutebrowser/config.py")
